...
{/*Average Information*/
	try {
		...
		if(numK2 > 0) {
			...
			for(size_t i=0; i<numK2; ++i) {
			    if(i == 0 && idxtau[0] == 0) {
					...
					AI(0, 0) = dot(PAPY, P * PAPY);
				} else {
				    PAPY = P * PHI.slice(idxtau[i]-1) * PY;
					score[i] = dot(Y, PAPY) - accu(P % PHI.slice(idxtau[i]-1));
					for(size_t j=0; j<=i; ++j) {
					    if(j == 0 && idxtau[0] == 0) {
						    AI(i,0) = dot(PY / D, PAPY);
							AI(0,i) = AI(i,0);
						} else {
						    AI(i,j) = dot(PY, PHI.slice(idxtau[j]-1) * PAPY);
		...
		return List::create(Named("tau") = tau, Named("P") = P, Named("cov") = XtHinvX_inv, Named("alpha") = alpha, Named("eta") = eta);
